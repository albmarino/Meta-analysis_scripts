...
# Show GC3 and species sharing in the GC-poor selected geneset for every dataset, filtering for minimum perc of gene sharing across species (Total and in each clade)

bashcmd <- paste0("sed '1d' ", args[4]," | awk '{FS=\"\\t\"} {if ($2/$3*100>=", args[3],") {print $0}}' | sort -k5| head -", as.numeric(args[1]), "| sort -r -k2 |head -", as.numeric(args[2]), "| cut -f1,2,3,5")
...
bashcmd <- paste0("sed '1d' ",args[4]," | awk '{FS=\"\\t\"} {if ($7/$8*100>=", args[3],") {print $0}}' | sort -k10| head -", as.numeric(args[1]), "| sort -r -k7 |head -", as.numeric(args[2]), "| cut -f1,7,8,10")
...
bashcmd <- paste0("sed '1d' ", args[4]," | awk '{FS=\"\\t\"} {if ($12/$13*100>=", args[3],") {print $0}}' | sort -k15| head -", as.numeric(args[1]), "| sort -r -k12 |head -", as.numeric(args[2]), "| cut -f1,12,13,15")
...
bashcmd <- paste0("sed '1d' ", args[4], " | awk '{FS=\"\\t\"} {if ($17/$18*100>=", args[3],") {print $0}}' | sort -k20| head -", as.numeric(args[1]), "| sort -r -k17 |head -", as.numeric(args[2]), "| cut -f1,17,18,20")
...
bashcmd <- paste0("sed '1d' ", args[4]," | awk '{FS=\"\\t\"} {if ($22/$23*100>=", args[3],") {print $0}}' | sort -k25| head -", as.numeric(args[1]), "| sort -r -k22 |head -", as.numeric(args[2]), "| cut -f1,22,23,25")
...
bashcmd <- paste0("sed '1d' ", args[4]," | awk '{FS=\"\\t\"} {if ($27/$28*100>=", args[3],") {print $0}}' |sort -k30| head -", as.numeric(args[1]), "| sort -r -k27 |head -", as.numeric(args[2]), "| cut -f1,27,28,30")
...
geom_boxplot()+
labs(y="Shared species in ref dataset (%)", title=paste0("Min ", args[3], "% of species sharing per gene in a dataset --> ", args[1], " GC3-poorest --> ", args[2], " top shared genes"), subtitle=paste0("Actinopteri=",nrow(df_species[df_species$Clade=="Actinopteri",]),"    Aves=", nrow(df_species[df_species$Clade=="Aves",]),   "    Insecta=", nrow(df_species[df_species$Clade=="Insecta",]), "    Mammalia=", nrow(df_species[df_species$Clade=="Mammalia",]),"    Mollusca=", nrow(df_species[df_species$Clade=="Mollusca",]),"    Total=", nrow(df_species)))+
...
#Write out lists of GC-poor selected genesets for each clade
write.table(subgc_tot$V1,file=paste0("gcpoorset_tot_", appliedfilt, "_",  args[3],"_",args[1],"_",args[2],".tsv"),sep="\n",row.names=F,col.names=F,quote=F)
write.table(subgc_act$V1,file=paste0("gcpoorset_actinopteri_",appliedfilt, "_",args[3],"_",args[1],"_",args[2],".tsv"),sep="\n",row.names=F,col.names=F,quote=F)
write.table(subgc_aves$V1,file=paste0("gcpoorset_aves_",appliedfilt, "_",args[3],"_",args[1],"_",args[2],".tsv"),sep="\n",row.names=F,col.names=F,quote=F)
write.table(subgc_ins$V1,file=paste0("gcpoorset_insecta_",appliedfilt, "_",args[3],"_",args[1],"_",args[2],".tsv"),sep="\n",row.names=F,col.names=F,quote=F)
write.table(subgc_mam$V1,file=paste0("gcpoorset_mammalia_",appliedfilt, "_",args[3],"_",args[1],"_",args[2],".tsv"),sep="\n",row.names=F,col.names=F,quote=F)
write.table(subgc_mol$V1,file=paste0("gcpoorset_mollusca_",appliedfilt, "_",args[3],"_",args[1],"_",args[2],".tsv"),sep="\n",row.names=F,col.names=F,quote=F)

pdf(paste0("GCpoor_subsets_", appliedfilt, "_", args[3], "_", args[1], "_", args[2], ".pdf"),width=11)
